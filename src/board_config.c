...
#define SOS_BOARD_CLOCK 96000000
...
const devfs_device_t devfs_list[] = {
	...
	DEVFS_DEVICE("random", random, 0, 0, &random_state, 0666, USER_ROOT, S_IFCHR),
...
const sysfs_t sysfs_list[] = {
	APPFS_MOUNT("/app", &mem_device, 0777, SYSFS_ROOT), //the folder for ram/flash applications
	DEVFS_MOUNT("/dev", devfs_list, 0777, SYSFS_ROOT), //the list of devices
	SFFS_MOUNT("/home", &sffs_cfg, 0777, SYSFS_ROOT), //the stratify file system on external RAM
...
const void * kernel_request_api(u32 request){
	if( request == JANSSON_API_REQUEST ){
		return &jansson_api;
	}
	return 0;
}
